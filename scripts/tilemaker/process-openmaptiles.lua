-- local_map: Kopie von tilemaker resources/process-openmaptiles.lua,
-- Stand systemed/tilemaker@aff30c67e6 (2026-08-29). Einzige Abweichung ist
-- mit "local_map:" markiert (residential-MinZoom). Beim Aktualisieren das
-- Original neu holen und die markierte Stelle wieder einsetzen.
	...
	-- Set 'landuse' (from landuse, amenity, leisure, tourism, place, waterway)
		...
		if landuseKeys[l] then
			...
			if l=="residential" then
				-- local_map: upstream setzt hier fuer jede Wohnflaeche unter
				-- ZRES8^2 (~0,37 km2) fest MinZoom(8). Damit steht jede Hofstelle
				-- schon in der Uebersichtskachel: rund 10.000 Flaechen je
				-- z8-Kachel ueber Hessen, 60 % ihres Inhalts, auf dem Pi 16 s bis
				-- die Ansicht steht. Nach Flaeche heisst: eine Flaeche erscheint
				-- ab der Zoomstufe, auf der sie einige Pixel gross ist. Grosse
				-- Flaechen bleiben wie bisher; Ortschaften bei z4-8 zeigt
				-- ohnehin urban_areas aus Natural Earth.
				SetMinZoomByArea()
			else MinZoom(11) end
			write_name = true
		end
